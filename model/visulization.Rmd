...
author <- read.csv ("updating_gender_bipoc_data.csv") %>% 
  select(author_full_names, publication_year, gender, bipoc) %>% 
  na.omit()
...
# Visualization
```{r}
ggplot(author_counts, aes(x = publication_year, y = count, color = interaction(gender, bipoc))) +
  geom_line(size = 1) +
  labs(title = "Author Appearances Across Years by Gender and BIPOC Status",
       x = "Publication Year",
       y = "Number of Authors",
       color = "Group (Gender & BIPOC)") +
  scale_color_manual(values = c("blue", "red", "green", "purple"),
                     labels = c("Male & Non-BIPOC", "Male & BIPOC", "Female & Non-BIPOC", "Female & BIPOC")) +
  theme_minimal()
